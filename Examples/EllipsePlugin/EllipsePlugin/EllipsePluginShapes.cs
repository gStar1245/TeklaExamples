...
namespace EllipsePlugin
{
    ...
    {
        internal static void DrawAnEllipse(ViewBase selectView, Point p1, Point p2, double majorAxis, double minorAxis, int precision)
        {
            View currentView = (View)selectView;
            ...
            List<Point> ellipsePoints = CalculateEllipsePoints(p1, angleOfRotation, majorAxis * scale, minorAxis * scale, precision);

            DrawEllipseWithArcs(precision, selectView, ellipsePoints);
        }
        ...
        internal static Point GetCenterPointOfArc(Point p1, Point p2, Point p3)
        {
            ...
            else
            {
                ...
                center.X = (aSlope * bSlope * (p1.Y - p3.Y) + bSlope * (p1.X + p2.X) - aSlope * (p2.X + p3.X)) /
                           (2 * (bSlope - aSlope));
                center.Y = -1 * (center.X - (p1.X + p2.X) / aSlope + (p1.Y + p2.Y) / 2.0);
                center.Z = p1.Z;
            }
        ...
        internal static List<Point> CalculateEllipsePoints(Point centerPoint, double rotationAngle, double majorAxis, double minorAxis, int steps)
        {
